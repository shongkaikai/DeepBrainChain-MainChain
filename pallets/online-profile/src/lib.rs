...
use frame_support::{
    ...
    pallet_prelude::*,
    traits::{Currency, ExistenceRequirement::KeepAlive, Get, LockableCurrency},
    weights::Weight,
...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn root_add_machine_info(
            ...
            let mut machine_info = machine_info;
            let now = <frame_system::Module<T>>::block_number();
            machine_info.bonding_height = now;
        ...
        pub fn root_add_stash_machine(
            ...
            let mut stash_machine = stash_machine;

            let mut linear_release = 0u32.saturated_into::<BalanceOf<T>>();
            for a_reward in stash_machine.linear_release_reward {
                linear_release += a_reward;
            }

            let mut linear = VecDeque::new();
            linear.push_back(linear_release);

            stash_machine.linear_release_reward = linear;
        ...
        #[pallet::weight(0)]
        pub fn root_add_controller_machines(
            origin: OriginFor<T>,
            controller: T::AccountId,
            controller_machines: Vec<MachineId>,
        ) -> DispatchResultWithPostInfo {
            ensure_root(origin)?;
        ...
        // #[pallet::weight(0)]
        // pub fn root_add_pos_info(
        //     origin: OriginFor<T>,
        //     pos_gpu_info: PosInfo,
        // ) -> DispatchResultWithPostInfo {
        //     ensure_root(origin)?;
        //     PosGPUInfo::<T>::insert();
        //     Ok(().into())
        ...
        #[pallet::weight(0)]
        pub fn root_set_stash_controller(
            origin: OriginFor<T>,
            stash: T::AccountId,
            controller: T::AccountId,
        ) -> DispatchResultWithPostInfo {
            ...
            StashController::<T>::insert(stash, controller);
            Ok(().into())
        }

        // TODO: should change current era and next era
        #[pallet::weight(0)]
        pub fn root_set_era_stash_points(
            origin: OriginFor<T>,
            era_index: EraIndex,
            ...
            ErasStashPoints::<T>::insert(era_index, era_stash_points);

            Ok(().into())
        }

        #[pallet::weight(0)]
        pub fn root_set_era_machine_points(
            origin: OriginFor<T>,
            era_index: EraIndex,
            era_machine_points: BTreeMap<MachineId, MachineGradeStatus<T::AccountId>>,
        ) -> DispatchResultWithPostInfo {
            ensure_root(origin)?;
            ErasMachinePoints::<T>::insert(era_index, era_machine_points);
            Ok(().into())
        }
        ...
        pub fn reonline_machine(
            ...
            // 重新上链需要交一定的手续费
            let stake_amount =
                T::DbcPrice::get_dbc_amount_by_value(Self::reonline_stake()).unwrap_or_default();
...
impl<T: Config> Pallet<T> {
    ...
    fn lc_confirm_machine_reonline(
        ...
        // 根据质押，奖励给这些委员会
        let reonline_stake = Self::user_reonline_stake(
    ...
    fn change_machine_status(
        ...
        match new_status {
            MachineStatus::Rented => {
                ...
                if let Some(new_gpu_num) =
                    stash_machine.total_rented_gpu.checked_add(machine_gpu_num)
                {
            ...
            MachineStatus::Online => {
                if rent_duration.is_some() {
                    ...
                    if let Some(new_gpu_num) =
                        sys_info.total_rented_gpu.checked_sub(machine_gpu_num)
                    {
                        sys_info.total_rented_gpu = new_gpu_num;
                    } else {
                    ...
                    if let Some(new_gpu_num) =
                        stash_machine.total_rented_gpu.checked_sub(machine_gpu_num)
                    {
                        stash_machine.total_rented_gpu = new_gpu_num;
                    } else {
